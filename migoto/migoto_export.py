...
# 导出到各个DrawIB文件夹中的ExportModel文件夹中
# TODO 后面改为从Blender中导出Mod后，这个就可以删掉了，暂时还得留着
def ExportToWorkSpace(self,context):
    ...
    for draw_ib_collection in workspace_collection.children:
        ...
        export_json_dict = CollectionUtils.parse_drawib_collection_to_export_json(draw_ib_collection)

        # Save to file.
        # TODO 未来直接从Blender输出Mod就不用这个了
        ...
        JsonUtils.SaveToFile(exported_folder_path + 'export.json',export_json_dict)
            ...
            for model_collection in export_component_collection.children:
                ...
                for obj in model_collection.objects:
                    ...
                    if obj.type == 'MESH' and obj.hide_get() == False:
                        ...
                        vb_path = exported_folder_path + obj.name + ".vb"
                        ib_path = os.path.splitext(vb_path)[0] + '.ib'
                        fmt_path = os.path.splitext(vb_path)[0] + '.fmt'
...
class DBMTExportAllToWorkSpace(bpy.types.Operator):
    ...
    def execute(self, context):
        # GlobalTimer.Start("Export All")
        result = ExportToWorkSpace(self,context)
        # GlobalTimer.End()
        return result
