...
class ModModel:
    ...
    def generate_unity_vs_config_ini(cls):
        ...
        for draw_ib, draw_ib_model in cls.drawib_drawibmodel_dict.items():
            ...
            vertex_limit_raise_filter_index = cls.add_vertex_limit_raise_section(ini_builder=ini_builder,draw_ib_model=draw_ib_model,generate_mod_number=cls.global_generate_mod_number)
            ...
            for count_i in range(len(draw_ib_model.part_name_list)):
                ...
                if toggle_type_number >= 2:
                    for toggle_count in range(toggle_type_number):
                        if toggle_count == 0:
                            texture_override_ib_section.append(vlr_filterindex_indent + "if $swapkey" + str(cls.global_key_index_logic) + " == " + toggle_count)
                        else:
                            texture_override_ib_section.append(vlr_filterindex_indent + "else if $swapkey" + str(cls.global_key_index_logic) + " == " + toggle_count)
                ...
                for switch_model_collection in switch_model_collection_list:
                    texture_override_ib_section.append(vlr_filterindex_indent + "if $swapkey" + str(cls.global_key_index_logic) + "  == 1")
